...
const getalluserCustumer = async function (req, res) {
  ...
  try {
    ...
    const { count, rows } = await Customers.findAndCountAll({
      ...
      offset: offset,
      order: [
        ["created_at", "DESC"], // เรียงตามวันที่สร้างล่าสุด (ตัวอย่าง)
...
const fetchAllTransactions = async function (req, res) {
  ...
  try {
    const page = parseInt(req.body.activePage) || 1; //  หน้าปัจจุบัน, เริ่มต้นที่ 1
    const limit = req.body.limit ? parseInt(req.body.limit) : 10; // จำนวนรายการต่อหน้า
    const offset = (page - 1) * limit; // คำนวณ offset
