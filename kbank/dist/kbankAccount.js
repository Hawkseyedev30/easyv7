...
class kbankAccount {
    ...
    getSession(dataRsso) {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                return resolve({ status: true, data: resp.data });
            }
            catch (e) {

                return resolve({
                    message: "Error during session getting",
                    status: false,
    ...
    getinquiryFundTransferAccount(prarams) {
        return new Promise(async (resolve, reject) => {
            try {
                let resp = await this.client.request({
                    method: "post",
                    jar: this.jar,
                    url: this.ib_gateway + '/api/bulkfundtransferbusiness/inquiryFundTransferAccount',
                    data: JSON.stringify({


                        "transType": prarams.transType,
                        "bankId": prarams.bankId,
                        "bankCode": prarams.bankCode,
                        "bankAbv": prarams.bankAbv,
                        "beneficiaryNo": prarams.beneficiaryNo,
                        "fromAccountNo": prarams.fromAccountNo,
                        "transferType": "Online",
                        "amount": 0,
                        'ownerId': this.sessions.custType == "IX" ? this.sessions.ownerIdIX : this.sessions.ownerId,
                        'ownerType': this.sessions.custType == "IX" ? this.sessions.ownerTypeIX : this.sessions.ownerType,
    ...
    getTransactionList(accountInfo, start_date, end_date, pageNo = 1, rowPerPage = 20) {
        return new Promise(async (resolve, reject) => {
            try {
                let currentTime = new Date();
                let startDateString = this.convertTime(currentTime);
    ...
    bankTransferOther(bankCode, id, amount, ownAccount) {
        return this.bankTransfer(bankCode, id, amount, ownAccount, `/api/fundtransferOther${this.sessions.custType == `I` ? `individual` : `business`}/inquiryFundTransferOther`, "FTOT");
    }
    ...
    getTransactionDetail(transactionInfomation, accountInfo, mobileNo) {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                payloadNew.acctNo = accountInfo.accountNo;

                payloadNew.mobileNo = mobileNo;
